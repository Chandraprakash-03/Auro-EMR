...
<div class="wrapper col4">
    <div id="container">
        ...
        if (isset($_POST['submit'])) {
            if (mysqli_num_rows($qsqlappointment) >= 1) {
                echo "<script>
                        alert('Appointment already scheduled for " . date("d-M-Y", strtotime($_POST['appointmentdate'])) . " " . date("H:i A", strtotime($_POST['appointmenttime'])) . "');
                      </script>";
            } else {
                if (isset($_SESSION['patientid'])) {
                    echo "<script>
                            alert('Appointment taken successfully.\\nAppointment record is in pending process. Kindly check the appointment status.');
                            window.location.href = 'viewappointment.php';
                          </script>";
                } else {
                    echo "<script>
                            alert('Appointment taken successfully.\\nAppointment record is in pending process. Please wait for confirmation message.');
